        ...
        for i in deelgebieden:
            for j in ["T25","T100"]:
                for k in ["25p","50p","75p","100p"]:
                    ...
                    b = b.replace('"','')
                    c = c.replace('"','')
                    if a == "Maastricht_West":
                        raster_names = {'dem_file': 'dem', 'frict_coef_file': 'friction', 'infiltration_rate_file': 'infiltration',  'initial_waterlevel_file': 'ini_waterlevel'}
                    elif a == "Banholt-Mheer" or a == "Rode_Beek" or a == "Parkstad_Geleenbeek" or a == "Parkstad_Worm": 
                        raster_names = {'dem_file': 'dem', 'frict_coef_file': 'friction', 'infiltration_rate_file': 'infiltration'}
                    elif a =="Geul_Oost":
                        raster_names = {'dem_file': 'dem', 'frict_coef_file': 'friction', 'infiltration_rate_file': 'infiltration','max_infiltration_capacity_file': 'max_infiltration'}
                    else:
                        raster_names = {'dem_file': 'dem', 'frict_coef_file': 'friction', 'infiltration_rate_file': 'infiltration',  'initial_waterlevel_file': 'ini_waterlevel', 'max_infiltration_capacity_file': 'max_infiltration'}
                    tags = ["Waterschap Limburg", "Stikstof", a, b, c]
                    print (a)
                    sqlite_path =  "/mnt/..." + a+ "/" + b +"/"+ c + "/schematisation/" + a + ' '+ b +'.sqlite'
                    schematisation_name = a +' '+ b+ ' ' + c
